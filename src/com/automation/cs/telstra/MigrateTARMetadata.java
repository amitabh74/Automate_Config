...
public class MigrateTARMetadata extends MigrateData implements IMigrateData{
    ...
    final static Logger logger = Logger.getLogger(MigrateTARMetadata.class);   

    /**
   	 * Extracts the data from Specification_Metadata__c and child object TAR_Metadata__c from source org
   	 * @param SourceConfig
   	 * @return String json
   	 */
	@Override
	...
	/**
	 * Get all Specification_Metadata__c from target sandbox
	 * @param conn
	 * @param trgConfig
	 * @return
	 ...
	 * @throws IOException
	 */
	public Map<String, String> getSpecificationMetadata(TargetConfig trgConfig) throws ClientProtocolException, IOException{
		Map<String, String> mapSpecMetadata = new HashMap<String, String>();
		
		String soqlQuery = "Select+ID+,+Name+From+Specification_Metadata__c";
        logger.debug("Query URL: " + soqlQuery);
            	 ...
            	 String response_string = null;
	              try {
	                  ...
	                  for (int i = 0; i < j.length(); i++){
	                  	...
	                  	mapSpecMetadata.put(obj.getString("Name"), obj.getString("Id"));
	                  }
	                  
	                  //Verify the values.
	                  /*for(Map.Entry<String, String> m : mapSpecMetadata.entrySet()){
        ...
        return mapSpecMetadata;
	}
	
	
	/**
	 * Insert Json to the TAR_Metadata__c object in target Org
	 * @param String json
	 */
	@Override
	public void insert(String sJson, TargetConfig trgConfig) {
		...
		//Map<Integer, String> mapSpecMetadata = new HashMap<Integer, String>();
		...
		try {
			...
			Map<String, String> mapTAR_SpecMetadata = getSpecificationMetadata(trgConfig);
            ...
            for (int i = 0; i < jRecords.length(); i++){
            	...
            	if(jRecords.getJSONObject(i).get("TAR_Metadata__r") != JSONObject.NULL){
            		 JSONObject childObj = jRecords.getJSONObject(i).getJSONObject("TAR_Metadata__r");
            		 logger.debug("Specification Name: " + obj.getString("Name"));
            		 
            		 if(mapTAR_SpecMetadata.get(obj.getString("Name")) != null){ //Check if Specification Name exists at current record index
            			 String Id = mapTAR_SpecMetadata.get(obj.getString("Name"));
            			 JSONArray specRecords = childObj.getJSONArray("records");
            			 //logger.debug("Specification Name: " + obj.getString("Name") + "  Id: " + Id);
            			 
            			 for(int k=0; k<specRecords.length(); k++){
            				 index++;
            				 specRecords.getJSONObject(k).put("Specification_Metadata__c", Id);
