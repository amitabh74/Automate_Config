...
public class MigrateSpecification extends MigrateData implements IMigrateData {
    ...
    final static Logger logger = Logger.getLogger(MigrateSpecification.class);
    
    /**
   	 * Extracts the data from csedm__Specification__c object from source org
   	 * @param SourceConfig
   	 * @return String json
   	 */
	@Override
	...
	/**
	 * Get all product definitions from target sandbox
	 * @param conn
	 * @param trgConfig
	 * @return mapProdDef
	 ...
	 * @throws IOException
	 */
	public Map<String, String> getProductDefintions(TargetConfig trgConfig) throws ClientProtocolException, IOException{
		Map<String, String> mapProdDef = new HashMap<String, String>();
		
		String soqlQuery = "Select+ID+,+Name+From+cscfga__Product_Definition__c";
	...
	/**
	 * Insert Json to the csedm__Specification__c object in target Org
	 * @param String json
	 */
	@Override
	public void insert(String sJson, TargetConfig trgConfig) {
		...
		try {
			...
			Map<String, String> mapProdDef = getProductDefintions(trgConfig);
			
			baseUri = conn.getLoginInstanceUrl() + REST_ENDPOINT + API_VERSION;
			String uri = baseUri + "/composite/tree/" + trgConfig.getEntity();  //csbb__Callout_Template__c";
            ...
            for (int i = 0; i < jRecords.length(); i++){
            	...
            	if(jRecords.getJSONObject(i).get("csedm__Product_Definitions__r") != JSONObject.NULL){
            		 JSONObject childObj = jRecords.getJSONObject(i).getJSONObject("csedm__Product_Definitions__r");
            		 logger.debug("Product Defintion Name: " + obj.getString("Name"));
            		 
            		 if(mapProdDef.get(obj.getString("Name")) != null){ //Check if Product Definition Name exists at current record index
            			 String Id = mapProdDef.get(obj.getString("Name"));
            			 ...
            			 for(int k=0; k<specRecords.length(); k++){
            				 index++;
            				 //Add the 
            				 //logger.debug("--Specification Name: " + specRecords.getJSONObject(k).getString("Name"));
            				 specRecords.getJSONObject(k).put("csedm__Product_Definition__c", Id);
